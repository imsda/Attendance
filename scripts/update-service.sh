...
ensure_service_started_on_exit() {
  if [[ "$SERVICE_STOPPED" -eq 1 ]]; then
    log "Starting service after failure (trap): chapel-attendance"
    if sudo systemctl start chapel-attendance; then
      log "Service started successfully in failure trap."
    else
      log "ERROR: Failed to start chapel-attendance in failure trap. Manual intervention required."
    fi
  fi
}
...
log "Starting Chapel Attendance safe update from repo root: $ROOT_DIR"
...
if [[ -z "${DB_URL}" ]]; then
  ...
  TS=$(date +%Y%m%d-%H%M%S)
  BACKUP_PREFIX="backend/backups/chapel-attendance-pre-update-$TS"
...
if npm run | grep -q "db:migrate"; then
  log "Stopping service: chapel-attendance"
  run_step "stop chapel-attendance systemd service" sudo systemctl stop chapel-attendance
  SERVICE_STOPPED=1
  ...
  log "Starting service: chapel-attendance"
  run_step "start chapel-attendance systemd service" sudo systemctl start chapel-attendance
  SERVICE_STOPPED=0
else
