...
const schema = z.object({
  ...
  chapelScanWindowEnabled: z.boolean().optional(),
  chapelScanStartTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'Start time must use HH:MM format.').optional(),
  chapelScanEndTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'End time must use HH:MM format.').optional(),
