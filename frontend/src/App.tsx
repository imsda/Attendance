...
function SyncPage() {
  ...
  useEffect(() => { void load(); }, []);
  function syncErrors(errors?: string[]) { return errors?.length ? ` First errors: ${errors.slice(0, 5).join(' | ')}${errors.length > 5 ? ` | ${errors.length - 5} more` : ''}` : ''; }
  async function sync() { setBusy(true); setMessage(''); try { const result = await api<{ imported: number; attendanceRowsAppended: number; summariesUpdated: number; failed: number; errors: string[] }>('/google/sync', { method: 'POST' }); setMessage(`Sync complete: ${result.imported} roster rows imported, ${result.attendanceRowsAppended} attendance rows exported, and ${result.summariesUpdated} summaries updated.${result.failed ? ` ${result.failed} rows failed.` : ''}${syncErrors(result.errors)}`); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Sync failed.'); } finally { setBusy(false); } }
  async function importRoster() { setBusy(true); setMessage(''); try { const result = await api<{ imported: number; failed: number; errors: string[] }>('/google/import-roster', { method: 'POST' }); setMessage(`Roster import complete: ${result.imported} rows imported.${result.failed ? ` ${result.failed} rows failed.` : ''}${syncErrors(result.errors)}`); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Roster import failed.'); } finally { setBusy(false); } }
  async function writeBack() { setBusy(true); setMessage(''); try { const result = await api<{ attendanceRowsAppended: number; summariesUpdated: number }>('/google/write-back', { method: 'POST' }); setMessage(`Write-back complete: ${result.attendanceRowsAppended} attendance rows exported and ${result.summariesUpdated} summaries updated.`); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Write-back failed.'); } finally { setBusy(false); } }
  ...
  return <>
    ...
    <div className="two-column"><section className="panel"><h2>Google Sheets</h2><div className={`status-pill ${status?.enabled ? 'good' : ''}`}>{status?.enabled ? 'Enabled' : 'Disabled'}</div><p>{status?.lastSyncSummary || 'No full sync has run yet.'}</p>{status?.lastSyncAt && <p className="muted">Last synced {formatDateTime(status.lastSyncAt)}</p>}<p className="muted">Credentials: {status?.credentialsConfigured ? 'configured' : 'not configured'} · Automatic sync: {status?.autoSyncEnabled ? 'on' : 'off'}</p><div className="button-row"><button className="primary" disabled={busy || !status?.enabled} onClick={() => void sync()}>{busy ? 'Working…' : 'Full sync'}</button><button className="secondary" disabled={busy || !status?.enabled} onClick={() => void importRoster()}>Import roster</button><button className="secondary" disabled={busy || !status?.enabled} onClick={() => void writeBack()}>Write back attendance & totals</button></div></section>
...
function Reports() {
  ...
  const [to, setTo] = useState(dateInput()); const [selectedMonth, setSelectedMonth] = useState(''); const [report, setReport] = useState<Report | null>(null); const [query, setQuery] = useState('');
  async function load() { setReport(await api<Report>(`/reports?from=${from}&to=${to}`)); }
  useEffect(() => { void load(); }, [from, to]);
  function preset(kind: 'week' | 'month' | 'year') { const now = new Date(); const start = new Date(now); if (kind === 'week') start.setDate(now.getDate() - ((now.getDay() + 6) % 7)); if (kind === 'month') start.setDate(1); if (kind === 'year') start.setMonth(0, 1); setSelectedMonth(''); setFrom(dateInput(start)); setTo(dateInput(now)); }
  function chooseMonth(value: string) { setSelectedMonth(value); if (!value) return; const year = new Date().getFullYear(); const month = Number(value); setFrom(dateInput(new Date(year, month, 1))); setTo(dateInput(new Date(year, month + 1, 0))); }
  ...
  return <><PageTitle title="Attendance reports" subtitle="Choose any date range or select a month from the current year." /><section className="panel"><div className="toolbar wrap"><button className="secondary" onClick={() => preset('week')}>This week</button><button className="secondary" onClick={() => preset('month')}>This month</button><button className="secondary" onClick={() => preset('year')}>This year</button><label>Month<select value={selectedMonth} onChange={(e) => chooseMonth(e.target.value)}><option value="">Select month</option>{Array.from({ length: 12 }, (_, month) => <option key={month} value={month}>{new Intl.DateTimeFormat(undefined, { month: 'long' }).format(new Date(2000, month, 1))}</option>)}</select></label><label>From<input type="date" value={from} onChange={(e) => { setSelectedMonth(''); setFrom(e.target.value); }} /></label><label>To<input type="date" value={to} onChange={(e) => { setSelectedMonth(''); setTo(e.target.value); }} /></label></div></section>
...
function SettingsPage() {
  ...
  return <><PageTitle title="Settings" subtitle="Configure the scanner, duplicate rules, and Google Sheet connection." /><form className="stack" onSubmit={save}><section className="panel"><h2>General</h2><div className="form-grid"><label>School name<input value={settings.schoolName} onChange={text('schoolName')} /></label><label>Timezone<input value={settings.timezone} onChange={text('timezone')} placeholder="America/Chicago" /></label><label>Station name<input value={settings.stationName} onChange={text('stationName')} /></label><label>Scanner cooldown (seconds)<input type="number" min="0.25" step="0.25" value={settings.scannerCooldownSeconds} onChange={(e) => setSettings({ ...settings, scannerCooldownSeconds: Number(e.target.value) })} /></label></div><div className="check-grid"><label className="checkbox"><input type="checkbox" checked={settings.oneAttendancePerDay} onChange={check('oneAttendancePerDay')} /> Count each student only once per day</label><label className="checkbox"><input type="checkbox" checked={settings.enableSounds} onChange={check('enableSounds')} /> Scanner sounds</label><label className="checkbox"><input type="checkbox" checked={settings.scannerDiagnosticsEnabled} onChange={check('scannerDiagnosticsEnabled')} /> Camera diagnostics</label></div></section>
    <section className="panel"><h2>Chapel check-in hours</h2><label className="checkbox"><input type="checkbox" checked={settings.chapelScanWindowEnabled} onChange={check('chapelScanWindowEnabled')} /> Only accept scans during the configured daily time window</label><div className="form-grid"><label>Start time<input type="time" value={settings.chapelScanStartTime} disabled={!settings.chapelScanWindowEnabled} onChange={text('chapelScanStartTime')} /></label><label>End time<input type="time" value={settings.chapelScanEndTime} disabled={!settings.chapelScanWindowEnabled} onChange={text('chapelScanEndTime')} /></label></div><p className="muted">Times use the configured academy timezone. A window such as 22:00–02:00 is supported when it crosses midnight.</p></section>
...
const roleAccess = (role: UserRole) => role === 'SCANNER' ? 'Scan Chapel' : role === 'REPORTER' ? 'Dashboard, Scan Chapel, Attendance Log, and Reports (read-only)' : role === 'ADMIN' || role === 'OWNER' ? 'Full administrative access' : 'Legacy custom access';
