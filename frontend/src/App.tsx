...
function SyncPage() {
  ...
  async function importRoster() { setBusy(true); setMessage(''); try { const result = await api<{ imported: number; failed: number }>('/google/import-roster', { method: 'POST' }); setMessage(`Roster import complete: ${result.imported} rows imported.${result.failed ? ` ${result.failed} rows failed.` : ''}`); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Roster import failed.'); } finally { setBusy(false); } }
